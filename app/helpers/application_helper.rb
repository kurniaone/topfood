module ApplicationHelper
  ...
  def link_to_add_items(name, f, association, el_id = '')
    new_object = f.object.class.reflect_on_association(association).klass.new
    fields = f.fields_for(association, new_object, :child_index => "new_#{association}") do |builder|
      render("orders/" + association.to_s.singularize + "_fields", :f => builder)
    end

    link_to_function(name, "add_items(this, \"#{association}\", \"#{escape_javascript(fields)}\")", :class => 'add-items link-button')
  end
